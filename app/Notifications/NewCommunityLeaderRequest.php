...
class NewCommunityLeaderRequest extends Notification
{
    use Queueable;
    ...
    public function __construct(User $user, $role_id)
    {
        ...
        $this->subject = 'Community leader request application';
        $this->first_message = "This is to notify that {$user->name} with user name {$user->user_name} has just requested to become {$role->name} in his community <a href='".route('community.profile', $community->id)."'>{$community->name}</a>.";
