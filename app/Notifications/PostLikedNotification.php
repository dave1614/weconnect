...
class PostLikedNotification extends Notification
{
     ...
     */
    public function __construct(User $liker, Post $post, User $user)
    {
        ...
        $this->subject = 'Your post was liked';
        $this->first_message = "This is to notify that your post made on {$post->created_at->format('d M Y')} with caption <em style='font-size: 14px; color: gray;'>{$post->caption}</em> was just liked by user <a class='text-blue-500 underline' target='_blank' href='".route('profile.show', $liker->slug)."'> {$liker->user_name}</a>";
