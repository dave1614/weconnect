          ...
          <table>
            ...
            <tbody>
              <tr v-for="(row,index) in wallet_statement.data" :key="row.id">
                ...
                <td :class="parseFloat(row.amount_after) > parseFloat(row.amount_before) ? 'text-green-500' : 'text-red-500'" data-label="Amount"  v-html="parseFloat(row.amount_after) > parseFloat(row.amount_before) ? '+' + mainStore.addCommas(row.amount) : '-' + mainStore.addCommas(row.amount)">
...
const submitFilterForm = () => {
  ...
  let queryRoute = route('wallet.statement', Object.keys(query).length ? query : {
    remember: 'forget'
  });
